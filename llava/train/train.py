...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        rank0_print(f"Loading emotion encoder from pretrained weights {expert_path}...")
        emotion_encoder = EmotionOV()
        emotion_encoder.load_zero_shot_weights(path=expert_path)

                                                                        
        emotion_encoder.freeze_layers()
        emotion_encoder.to(dtype=torch.bfloat16 if training_args.bf16 else torch.float16, device=training_args.device)
                                    
        emotion_encoder.requires_grad_(False)

        model.set_emotion_encoder(emotion_encoder)
        ...
        if model_args.mm_tunable_parts is None:                                                   
            ...
            if "mm_mlp_adapter" in tunable_parts:
                for p in model.get_model().mm_projector.parameters():
                    p.requires_grad = True
            if "emotion_encoder" in tunable_parts:
                for p in model.get_model().emotion_encoder.parameters():
                    p.requires_grad = True
            ...
            if "mm_language_model" in tunable_parts:
                for name, param in model.named_parameters():
                    if "vision_tower" not in name and "mm_projector" not in name and "vision_resampler" not in name and "emotion_encoder" not in name:
                        param.requires_grad_(True)
