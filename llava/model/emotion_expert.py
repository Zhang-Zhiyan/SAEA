...
class EmotionOV(nn.Module):
    def __init__(self, embedding_dim=512, class_dim=512, kernal_size=3, attention_dim=512, num_pooling_tokens=2, num_emotions=9, num_emotion_embeddings=2):
        super().__init__()
        ...
        self.num_emotion_embeddings = num_emotion_embeddings          
    ...
    def _build_emotion_components(self):
        ...
        self.emotion_embeddings = nn.Parameter(
            torch.randn(self.num_emotion_embeddings, self.embedding_dim),
            requires_grad=True
        )
    ...
    def load_zero_shot_weights(self, path='./checkpoints_stage1/emotion_expert_stage1_best.pth'):
        """加载预训练的情感专家权重"""
        ...
        if 'emotion_expert_state_dict' in checkpoint:
            ...
            print(f"📊 Loading from nested checkpoint structure")
            ...
            print(f"📊 Loading from direct state dict")
        ...
        if len(matched_names) > 0:
            ...
            print("⚠️  No matching keys found, using random weights")
    ...
    def get_emotion_map(self, sig_multi_level_features, ov_base_features): 
        ...
        base_emotion_embeddings = self.emotion_embeddings.unsqueeze(0).repeat(total_size, 1, 1)                                                

                      
                         
                           
        emotion_embeddings = base_emotion_embeddings                                                
        ...
        for i in range(4):        
            emotion_prompts = []
            for emotion_idx in range(self.num_emotion_embeddings):             
        ...
        for i in range(4):
            ...
            attention_map = attention_scores.permute(0, 2, 1).view(total_size, self.num_emotion_embeddings, 27, 27
            )                                         

                                        
            attention_map = torch.softmax(attention_map.view(total_size, self.num_emotion_embeddings, -1), dim=1)
            attention_map = attention_map.view(total_size, self.num_emotion_embeddings, 27, 27)

            emotion_attention_maps.append(attention_map[:, 1:, :, :])

                                
            patch_scores = attention_map[:, 1:, :, :].mean(dim=(1,2, 3))                                 
